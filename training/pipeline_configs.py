...
# epochs=100 is an upper bound, not a target -- train_model's early stopping (patience, see
# notebooks/train_unet_landuse.ipynb) usually halts well before that. Raised from 20, then
# 60, then 100 as the AOI list grew more visually diverse (more regions): with 17 AOI across
# very different parts of France, val mean IoU is noisy and doesn't clearly plateau within
# 20-30 epochs the way it did on the earlier, more homogeneous (mostly Nord-Pas-de-Calais)
# dataset -- see the run comparison in the notebook's section 5 discussion.
