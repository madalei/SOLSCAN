...
9. **Définition de Parking élargie à l'entraînement, restreinte seulement au résultat final** -- `rasterize_landuse_mask`'s `min_parking_area_m2` vaut désormais `0` par défaut (`helpers/mask_rasterize.py`) : tous les parkings, petits ou grands, sont étiquetés Parking dans les masques d'entraînement, plus seulement ceux ≥1500m². Le seuil loi APER s'applique maintenant en post-traitement sur la prédiction (`helpers/mask_postprocess.filter_small_regions`, basé sur des composantes connexes via `scipy.ndimage.label`, branché dans `api/segmentation_inference.py`), pas sur la vérité-terrain d'entraînement. Rationale : filtrer les petits parkings à la génération des masques jetait la majorité des exemples disponibles pour une classe déjà la plus rare du dataset, alors qu'un petit parking a la même signature visuelle (bitume, marquage, voitures) qu'un grand -- l'ancien filtre enseignait une frontière que le modèle ne peut pas voir dans les pixels. **Pas encore mesuré** : implémenté mais aucun run d'entraînement sur le dataset regénéré avec cette définition élargie n'a encore eu lieu au moment de l'écriture de ce document -- l'IoU Parking mesuré dans `train_unet_landuse.ipynb` après ce changement portera sur "détecte-t-on un parking de toute taille", pas directement comparable aux chiffres ci-dessous (voir note ajoutée section 7 de ce notebook).

### Conclusion sur Parking (jusqu'à l'item 8 inclus -- item 9 non encore mesuré)

**Malgré tous les leviers 1 à 8 ci-dessus, l'IoU Parking n'a jamais dépassé ~0.03 sur aucun run, et reste le plus souvent entre 0.005 et 0.02** -- y compris le run avec le plus de pixels Parking jamais atteint (poids de classe le plus bas, 57.3). Ce n'est plus attribuable à l'overfitting, au déséquilibre de classe non corrigé, ou au volume de données : ces trois causes ont été adressées et l'IoU n'a pas bougé de façon significative pour autant.

Diagnostic retenu à ce stade : **plafond structurel de résolution**, pas un problème de réglage. Un parking de 1500m² (seuil loi APER, le minimum ciblé) ne fait que ~15 pixels à 10m/pixel (Sentinel-2) -- proche du plancher de ce qu'un U-Net avec encodeur ResNet34 (downsampling ×32 au niveau le plus profond) peut apprendre à localiser de façon fiable, quel que soit le volume/la diversité des exemples fournis.

**Recommandation avant l'item 9** : ne pas continuer à itérer sur les leviers données/entraînement pour Parking spécifiquement -- rendement décroissant démontré empiriquement. Pistes qui *pourraient* encore aider mais changent significativement le scope :
- ~~Élargir la définition de Parking à l'entraînement~~ -- fait (item 9), à mesurer.
